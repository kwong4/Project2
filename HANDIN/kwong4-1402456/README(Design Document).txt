...
Some Notes:
- Also note, sometimes the printing of the Client won't print, however the correct action has been received. Just type any character to verify the speed of the Client (As it doesn't lag, just doesn't print quick enough). So for example you could type space really fast after it is suppose to accept another input and it would verify that it is not stalled, just having a bit of terminal updating issues. (Outside of program). So although the Client may seemingly lag, it doesn't, just type any character and it will give immediate response. Just remove the extra character typed if you don't want it to be entered. DON'T USE ENTER as a test, as it will send a input of 0 characters which is out of bounds of the inputs of the program (>= 2). This delay is usually after you enter the password and after receiving a file. Only occurs sometimes.
